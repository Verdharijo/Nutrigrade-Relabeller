...
selected = option_menu(
	menu_title="NUTRIGRADE+: Take Your Labels With A Pinch of Salt",
	options=["Nutritional Labelling", "Real-Time Scanner", "Product Recommender"],
	icons=['list', 'eyeglasses', 'cup'],
	default_index = 0,
...
if selected =="Nutritional Labelling":
    st.markdown("<h1 style='text-align: center;'>#Beyond The Nutrition Labels</h1>", unsafe_allow_html=True)
    left_co,last_co = st.columns(2)
    with left_co:
        st.image("https://healthscreening.sg/img/nutri-grade-labels.webp", caption = "Fig 1: Current HPB Nutrigrade System", width = 470)
        st.subheader("Nutrigrade Summary")
        st.write("The Health Promotion Board (HPB) decided to introduce mandatory nutrition labels and advertising prohibitions for Nutri-Grade beverages, after carefully considering the feedback received from public, industry and expert stakeholders, and reviewing existing overseas and local evidence.  Non-compliance with the measures will be an offence punishable upon conviction with a fine not exceeding $1,000. A summary of the new requirements for Nutri-Grade measures can be found in the [Annex](https://www.moh.gov.sg/docs/librariesprovider5/default-document-library/annexdd9aeff5263346b3b855d3eecb5c0be2.pdf).")
    with last_co:
        st.image("https://youthopia.sg/wp-content/uploads/2022/12/hpb-nutrition-grading-body-image.png", width = 700, caption = "Fig 2: Greater Details on Saturated Fat Benchmarks and Conditions")
        st.subheader("Policy Implications")
        st.write("Advertisements of Nutri-Grade beverages graded \"D\", are prohibited across all media platforms (e.g. broadcast, print, out-of-home, on-ground, online) except for advertisements of pre-packaged beverages at point-of-sale (POS) platforms within variety shops (e.g., supermarkets, convenience stores). Please refer to the Food Regulations [here](https://sso.agc.gov.sg/SL/SFA1973-RG1?DocDate=20190830) for the relevant legislation effecting the Nutri-Grade measures on pre-packaged beverages and non-customisable beverages dispensed from automated beverage dispensers since 30 December 2022. ")
    ...
    with tab1:
        ...
        with col2:
            ...
            st.write("By including a sodium label, this allows consumers and HPB to track potential outliers within these healthier products. For example, mineral waters can have a large range of sodium content with mineralized soidum content reaching up beyond 10mg/100mL.")
...
if selected == "Real-Time Scanner":
    ...
    with col2: 
        ...
        if activate:
            ...
            st.subheader("Nutrigrade+ Classification: D")
...
if selected == "Product Recommender":
    ...
    drink_name = st.text_input('Example: \"pokka bottle drink - peach tea\"', placeholder = "pokka")
